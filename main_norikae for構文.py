...
networkTmpl = templateEnv.get_template("norikae2.html")  # 乗換案内用のテンプレートを"norikae.html"から読み込む。
...
class Norikae(webapp2.RequestHandler):
    def get(self):
        ...
        self.response.headers['Content-Type'] = 'text/html; charset=UTF-8'
        self.response.write(networkTmpl.render(train_line=train_line, request=self.request))
